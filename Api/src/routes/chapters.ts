...
chaptersRouter.post<{}, {}>("/", upload.fields([{name: 'portada', maxCount: 1}, {name: 'chapters', maxCount:20}]), async (req, res, next) => {
  ...
  const newChapter = new Chapter(title, images, cover, Number(mangaId));
  try {
...
chaptersRouter.put("/chapter/updateImages/:idChapter", upload.array('chapters', 20), async (req, res, next) => {
...
chaptersRouter.put("/chapter/updateCover/:idChapter", upload.single('cover'), async (req, res, next) => {
  const {idChapter} = req.params;
...
chaptersRouter.get<{ idChapter: string }, {}>( "/:idChapter", async (req, res, next) => {
    const { idChapter } = req.params;
...
chaptersRouter.get("/chapter/image/:idChapter/:imageIndex", async (req, res, next) => {
